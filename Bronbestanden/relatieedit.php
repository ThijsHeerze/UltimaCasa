...
$ID = isset($_GET["rel"]) ? $_GET["rel"] : null;
...
$sql = "SELECT Naam, Email, Telefoon, FKrollenID FROM relaties WHERE relaties.ID = :ID";
$stmt = $db->prepare($sql);
$stmt->bindParam(':ID', $ID, PDO::PARAM_INT);
$stmt->execute();
$gegevens = $stmt->fetch();

$sql = "SELECT ID, CONCAT(Naam, ' - ', Omschrijving) AS Rol FROM rollen ORDER BY Waarde";
...
echo '</select>
                ...
                <div class="form-group">
                    <button type="submit" class="action-button" id="upd" name="upd" value="' . $ID . '" title="De rol van deze relatie aanpassen.">Wijzigen</button>
